...
describe("companyRegistryStore", () => {
  ...
  it("drops traceability payloads that do not match the current envelope schema (v13 stale)", () => {
  ...
  it("merges shared registry records without discarding richer local peer state", () => {
    ...
    expect(merged.companies.ITC?.traceability?.schemaVersion).toBe(TRACEABILITY_SCHEMA_VERSION);
  });
});
