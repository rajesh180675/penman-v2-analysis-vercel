/* ================================================================
   Plan 3 PR-3.4 — NBFC pipeline strategy.

   Same shim pattern as bank/industrial. NBFC routes through
   processBankData (which carries the NBFC + insurance subtype
   detection internally). matches() fires only when
   config.company_type === "nbfc"; "auto" NBFCs flow through the
   industrial-shim, identical behaviour to today.
================================================================ */
...
export class NbfcPipelineStrategy implements PipelineStrategy {
  readonly id = "nbfc-v1";
  readonly kind = "nbfc" as const;
  readonly version = "1.0.0";

  matches(_rawData: RawPeriodData[], config: EngineConfig): boolean {
    void _rawData;
    return config.company_type === "nbfc";
  }
  ...
  detectAnomalies(_rawData: RawPeriodData[], _recastData: RecastPeriod[]): AnomalyReport {
    void _rawData;
    void _recastData;
    return { flags: [], details: { note: "NBFC anomalies surfaced via bankResult.bankMetrics" } };
  }

  value(_input: ValuationInput): ValuationResult {
    void _input;
    throw new Error(
      "NbfcPipelineStrategy.value(): not implemented in canary. NBFC valuation runs inside processBankData() until PR-3.5 lands the orchestrator switch.",
    );
  }
  ...
  contributeToEnvelope(ctx: EnvelopeContext): SectorEnvelopeContribution {
    ...
    return {
      sectorStatus: {
        ...
        reasons: scopeBlocked ? ["NBFC scope assessment blocked"] : [],
      },
      sectorBlocks: { strategy: this.id, version: this.version },
    };
  }

  /** NBFC-specific helper exposed for the orchestrator (PR-3.5). */
  runNbfcPipeline(rawData: RawPeriodData[], config: EngineConfig) {
...
registerStrategy(new NbfcPipelineStrategy());
