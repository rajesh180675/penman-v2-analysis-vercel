/**
 * Advanced Models — Larsen & Toubro (Conglomerate with segments)
 *
 * @vitest-environment jsdom
 *
 * Verifies all Sprint 1 advanced models produce valid results for L&T.
 */
...
const ZIP_PATH = resolve(COMPANIES_DIR, "Larsen & Toubro Ltd", "Larsen & Toubro Ltd.zip");
const HAS_ZIP = existsSync(ZIP_PATH);

describe.skipIf(!HAS_ZIP)("Advanced Models — Larsen & Toubro (Conglomerate)", () => {
  ...
  beforeAll(async () => {
    const zipPath = resolve(COMPANIES_DIR, "Larsen & Toubro Ltd", "Larsen & Toubro Ltd.zip");
    const buf = readFileSync(zipPath);
    const file = new File([buf], "Larsen & Toubro Ltd.zip", { type: "application/zip" });
