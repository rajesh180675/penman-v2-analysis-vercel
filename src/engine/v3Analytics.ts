...
══════════════════════════════════════════════════════════════════ */
// Moved to ./v3Analytics/triggers (imports DOWN from ../types,
// ./eventFraming, ./shared, ./mathUtils; no back-edge). Re-exported here so
// external import paths (supplementaryPathA.spec) are unchanged. The
// MonitoringTrigger type is also used locally below, hence the type import.
import {
  calibrateMonitoringTriggers,
  generateMonitoringTriggers,
  type MonitoringTrigger,
} from "./v3Analytics/triggers";
export { calibrateMonitoringTriggers, generateMonitoringTriggers };
export type { MonitoringTrigger };
/* ══════════════════════════════════════════════════════════════════
   §5.9 Summary statistics helpers
══════════════════════════════════════════════════════════════════ */
/* report-rendering helpers (§5.9 ratio summary, OA decomposition, S-15.3
   accrual table) moved to ./v3Analytics/reporting; imported DOWN. Parent
   re-exports so external paths (V3AnalyticsPanel, supplementaryPathA.spec)
   are unchanged. */
import {
  buildRatioSummary,
...
export type { RatioSummary, OADecompositionResult, AccrualTableRow };
