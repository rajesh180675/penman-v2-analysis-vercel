...
function mkRatioPeriod(period_end: string): RecastPeriod {
  return {
    period_end,
    bs: {
      TA: 1000,
...
describe("RatioReport", () => {
  it("renders the shared traceability trust gate ahead of ratio analysis", () => {
    const data = [
      mkRatioPeriod("2024-03-31"),
      mkRatioPeriod("2025-03-31"),
    ];
    ...
    const html = renderToStaticMarkup(<RatioReport data={data} traceability={traceability} />);

    expect(html).toContain("Ratio Trust Gate");
    expect(html).toContain("Economically plausible");
    expect(html).toContain("ROCE Decomposition");
  });
});
