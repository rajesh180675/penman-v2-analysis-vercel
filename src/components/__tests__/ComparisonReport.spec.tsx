...
import { DEFAULT_CONFIG, CompanyRegistry, RecastPeriod, RawPeriodData } from "../../engine/types";

function mkRawPeriod(companyId: string, period_end: string, revenue = 900): RawPeriodData {
  return {
    ...
    raw_metric_values: {
      ...
      "Revenue From Operations(Net)__ProfitLoss": revenue,
      "Profit After Tax__ProfitLoss": 90,
    },
  };
}

function mkRecastPeriod(period_end: string, sales = 900): RecastPeriod {
  return {
    period_end,
    bs: {
      TA: 1000,
...
function mkTraceability(companyId: string, sales: number) {
  const rawData = [
    mkRawPeriod(companyId, "2024-03-31", sales),
    mkRawPeriod(companyId, "2025-03-31", sales),
  ];
  const recastData = [
    mkRecastPeriod("2024-03-31", sales),
    mkRecastPeriod("2025-03-31", sales),
  ];
...
describe("ComparisonReport", () => {
  it("renders a trust section before comparison tables using per-company traceability", () => {
    const registry: CompanyRegistry = {
      companies: {
        ITC: {
          id: "ITC",
          label: "ITC",
          rawData: [mkRawPeriod("ITC", "2024-03-31"), mkRawPeriod("ITC", "2025-03-31")],
          recastData: [mkRecastPeriod("2024-03-31"), mkRecastPeriod("2025-03-31")],
          traceability: mkTraceability("ITC", 900),
        },
        HUL: {
          id: "HUL",
          label: "HUL",
          rawData: [mkRawPeriod("HUL", "2024-03-31", 1000), mkRawPeriod("HUL", "2025-03-31", 1000)],
          recastData: [mkRecastPeriod("2024-03-31", 1000), mkRecastPeriod("2025-03-31", 1000)],
          traceability: mkTraceability("HUL", 1000),
        },
      },
    };

    const html = renderToStaticMarkup(<ComparisonReport registry={registry} config={DEFAULT_CONFIG} />);

    expect(html).toContain("Comparison Trust Gate");
    expect(html).toContain("Per-Company Trust State");
    expect(html).toContain("ITC");
    expect(html).toContain("HUL");
    expect(html).toContain("Parser fidelity");
    expect(html).toContain("Valuation Comparison Panel");
  });
});
