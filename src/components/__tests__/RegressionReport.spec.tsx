...
function mkRegressionPeriod(period_end: string): RecastPeriod {
  return {
    period_end,
    bs: {
      TA: 1000,
...
describe("RegressionReport", () => {
  it("renders the shared traceability trust gate ahead of regression deltas", () => {
    const recastData = [
      mkRegressionPeriod("2024-03-31"),
      mkRegressionPeriod("2025-03-31"),
    ];
    const rawData = [
    ...
    const traceability = buildAnalysisTraceability({
      generatedAt: "2026-04-03T13:00:00.000Z",
    ...
    expect(html).toContain("Regression Trust Gate");
    expect(html).toContain("Economically plausible");
    expect(html).toContain("Post-Fix Regression Harness");
  });
});
