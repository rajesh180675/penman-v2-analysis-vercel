...
The `AnalysisTraceabilityEnvelope` is the shared confidence signal consumed across all UI tabs. The current schema is exported from `src/engine/policyVersions.ts` as `TRACEABILITY_SCHEMA_VERSION` (currently `2026-06-traceability-v22`); persisted envelopes flow through `src/lib/envelopeMigrations.ts` for forward migration. It contains:
